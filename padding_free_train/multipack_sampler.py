...
class MultipackDistributedBatchSampler(BatchSampler):
    ...
    def set_epoch(self, epoch: int):
        logger.info(f"rank {self.rank} multi pack sampler set_epoch: {epoch}")
        self.epoch = epoch
    ...
    def generate_batches(self, set_stats=False):
        ...
        batches = [indices[batch].tolist() for batch in batches]
    ...
    def __iter__(self) -> Iterator[List[int]]:
        ...
        logger.info(
            f"rank {self.rank} multi pack sampler generate_batches length: {len(batches)}, efficiency: {self.efficiency()}"
        )
        return iter(batches)
